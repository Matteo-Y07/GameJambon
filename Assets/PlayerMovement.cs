...
public class PlayerMovement : MonoBehaviour
{
    ...
    void KeyboardInput()
    {
        ...
        if (Input.GetKey("Horizontal") && !isDashing) playerMoveRight();

        else if (Input.GetKey("Horizontal") && !isDashing) playerMoveLeft();

        else if (!isDashing) playerStop();

        // Dash
        if (Input.GetKeyDown(KeyCode.Space) && !isGrounded && !hasJump && hasDash) StartCoroutine(playerDash());
    ...
    IEnumerator playerDash()
    {
        ...
        if (Input.GetKeyDown("Horizontal") && Input.GetAxisRaw("Horizontal") > 0) dashDirection.x += 1f;   // droite
        if (Input.GetKeyDown("Horizontal") && Input.GetAxisRaw("Horizontal") < 0) dashDirection.x -= 1f;   // gauche
        if (Input.GetKeyDown("Vertical") && Input.GetAxisRaw("Vertical") > 0) dashDirection.y += 1f;   // haut
        if (Input.GetKeyDown("Vertical") && Input.GetAxisRaw("Vertical") < 0) dashDirection.y -= 1f;   // bas
